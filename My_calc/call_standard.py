...
class StandardPageWindow(QWidget,Ui_standard):
        ...
        if value == 'b' or value == 's' or  value == '^' or value == '%':
            if value == 'b':
                ...
                if eval(self.mid) != 0:
                    ...
                    self.displayclear()
                    self.display_error("Знаменатель не может быть равен нулю")
                    return
            elif value == '^':
                if len(self.mid) == 0 or self.s == 1:
                    return
                ...
                if eval(self.mid) >= 0:
                    ...
                    self.displayclear()
                    self.display_error("Число в корне должно быть больше или равно нулю")
                    return
            elif value == '%':
    ...
    def calculation(self):
        ...
        try:
            ...
            self.display_error("Math Error : Деление на ноль")
        except ValueError:
            self.display_error("Math Error : Нельзя использовать отрицательные числа в sqrt/log ,cos-1 & sin-1 ")
        except SyntaxError:
            self.display_error("Введено неверное уравнение")
        except AttributeError:
            self.display_error("Ошибка ввода: Пожалуйста, введите правильно")
        else:
